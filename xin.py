...
class ShapeMatcher:
    ...
    @staticmethod
    def propose_candidate_boxes(image_rgb, min_area_pct=0.02, max_area_pct=0.45):
        """
    ...
    def segment_box(self, image_rgb, box_coords):
        ...
        if self.predictor is not None:
            try:
                ...
                # Get predictions (SAM returns multiple candidate masks)
                ...
                # Convert torch outputs to pure binary numpy arrays [0, 255]
                results = []
                for m in masks:
                    bin_m = (m * 255).astype(np.uint8)
...
class DXFtoSAMPipelineUI:
    ...
    def load_dxf_template(self):
        ...
        try:
            ...
            # Render to left preview
            self.render_mask_on_canvas(self.dxf_solid_mask, self.canvas_dxf_preview)
            self.set_status("Nạp và làm đặc DXF thành công.")
            
        except Exception as e:
            messagebox.showerror("Lỗi Đọc DXF", f"Không thể sinh mask: {e}")
            print(traceback.format_exc())
    ...
    def run_auto_scan_process(self):
        """Algorithmic searching pipeline executing fully asynchronous in a background thread."""
        try:
            img = self.current_image_rgb
            self.root.after(0, lambda: self.set_status("🔍 Đang trích xuất vùng ứng viên...", "#ffea00"))
            
            # 1. Generate intelligent candidate bounding boxes using geometry
            candidates = ShapeMatcher.propose_candidate_boxes(img)
            
            count = len(candidates)
            self.root.after(0, lambda: self.set_status(f"Đã tìm thấy {count} ứng viên. Bắt đầu chạy AI...", "#ffea00"))
            ...
            # 2. Sequentially run segmentation + scoring on candidate proposals
            for i, box in enumerate(candidates):
                self.root.after(0, lambda idx=i: self.set_status(f"🧠 Đang quét đối tượng {idx+1}/{count}...", "#ffea00"))
                
                # Run SAM prediction (this uses AI GPU/CPU instantly)
                masks = self.sam_wrapper.segment_box(img, box)
                if not masks: 
                    continue
                
                # Calculate score for each segmented candidate mask
                for mask in masks:
                    # ⚡ LỌC BẪY NHIỄU TRONG HỘP LỚN: Loại mặt nạ nếu kích thước thực tế < 15% chiều ngang / 12% chiều dọc ảnh
                    cnt_m = ShapeMatcher.get_normalized_contour(mask)
                    if cnt_m is not None:
                        _, _, mw, mh = cv2.boundingRect(cnt_m)
                        if mw < img.shape[1] * 0.15 and mh < img.shape[0] * 0.12:
                            continue # Hạt bụi bé tí chui vào hộp to sẽ bị loại thẳng tay!
                            
                    curr_iou, curr_hu, curr_warped = ShapeMatcher.align_and_compute_iou(self.dxf_solid_mask, mask)
                    
                    # We select the box that maximizes our Shape Overlap IoU!
            ...
            # =====================================================================
            # 🔄 SIÊU CẤP TINH CHỈNH LỚP 2 (TWO-PASS TIGHT REFINER) 🔄
            # =====================================================================
            # Tại sao anh kéo tay lại đẹp hơn Auto? Vì Hộp Mỏ Neo (Anchor) của Auto quá rộng, 
            # khiến SAM2 bị phân tán sự chú ý. Khi anh kéo tay khít khao, SAM2 hội tụ 100% sức mạnh!
            # THUẬT TOÁN NÀY SẼ TỰ ĐỘNG GIẢ LẬP CÚ KÉO TAY KHÍT KHÌN KHỊT CỦA ANH:
            # Ta lấy Mặt nạ thắng cuộc ở Vòng 1 -> Tự động bóp Hộp giới hạn Siêu Khít (Tight Box) 
            # -> Chạy SAM2 Lần 2 duy nhất trên Hộp Khít này! Kết quả sẽ đạt điểm 88% Tối Đa!
            # =====================================================================
            if best_iou > 0.20 and best_mask is not None:
                ...
                if cnt is not None:
                    ...
                    # Thêm 10% Padding khít khao giả lập hoàn hảo hành động kéo chuột của con người!
                    pad_w = int(rw * 0.10)
                    pad_h = int(rh * 0.10)
                    ...
                    # Chạy duy nhất 1 pass AI Refinement siêu tốc (~50ms) trên hộp khít:
                    ...
                    if refined_masks:
                        for ref_m in refined_masks:
                            ...
                            # Nếu Hộp khít mang lại IoU cao hơn hoặc tương tự, ta cập nhật ngay làm kết quả cuối cùng!
                            if r_iou > best_iou or (r_iou > 0.75):
                                best_iou = r_iou
                                best_hu = r_hu
                                best_mask = ref_m
                                best_box = tight_box
            ...
            # 3. Render results if successful
            if best_iou > 0.0 and best_box is not None:
                ...
                self.root.after(0, lambda: self.set_status("❌ Không tự động tìm thấy cấu trúc Logo Nike.", "#ff5252"))
                
        except Exception as err:
            print(traceback.format_exc())
            self.root.after(0, lambda: self.set_status("❌ Gặp sự cố khi tự động phân tích ảnh.", "#ff5252"))
